        ...
        <div class="row">
            ...
            <div class="spana9">
                <ul class="breadcrumb" style="background-color: #1a4bbb;color:white">
                    ...
                    <li>Expired Business Listing</li>
                </ul>
                ...
                <div class="well">
                    ...
                    @foreach($row['listingid'] as $listid)
                    @php
                    $listingFesspage =App\Models\LisitngFees::where('id','1')->where('id',$listid->listing_id)->first();
                    ...
                    @if($day > $listingFesspage['duration'])
                    <table class="table">
                        <tr class="mt-5">
                            <th>
                                <img class="img" src="{{asset('images/listing/'.$row['logo'])}}" height="180px"
                                    width="180px">
                            </th>
                            <td>
                ...
                <div><i class="fas fa-tachometer-alt fa-fw"></i><span style="color:red;">{{$row['address']}}</span></div> -->
                            ...
                            <td>
                                ...
                                <div class="sec-center" style=" margin-top: 110px;
                        margin-left: -100px;">
                                    <input class="dropdown" type="checkbox" id="dropdown{{$row->id}}"
                                        style="display: none;" name="dropdown" />
                                    <label class="for-dropdown" for="dropdown{{$row->id}}"
                                        style="position: absolute;margin-top: -47px;left: -43px;"><i
                                            class="icon-cog"></i> <i class="uil uil-arrow-down"></i></label>
                                    <div class="section-dropdown" style="width: 169px;top: 0px;">
                                        <a href="{{ route('user.listing.view',$row->id) }}" class="dm">View Expired
                                            Lisitng <i class="uil uil-arrow-right"></i></a>
                                        <a href="{{route('user.listing.edit',$row->id)}}" class="dm">Edit Expired
                                            Lisitng <i class="uil uil-arrow-right"></i></a>
                                        <a href="{{route('user.listing.edit',$row->id)}}" class="dm">Expired Relist <i
                                                class="uil uil-arrow-right"></i></a>
                                        <a href="{{route('user.listing.delete',$row->id)}}" class="dm">Delete Expired <i
                                                class="uil uil-arrow-right"></i></a>
                                    </div>
                                </div>
